...
exports.getHint = exports.resolveSchema = exports.VIRTUAL_JSON_URL = void 0;
...
function parseYAML(schema) {
    try {
        ...
        (0, utils_js_1.error)(`YAML: ${String(err)}`);
        process.exit(1);
    }
}
function parseJSON(schema) {
    try {
        ...
        (0, utils_js_1.error)(`JSON: ${String(err)}`);
        process.exit(1);
    }
}
function resolveSchema(filename) {
    if ((0, utils_js_1.isRemoteURL)(filename))
        return new node_url_1.URL(filename.startsWith("//") ? `https:${filename}` : filename);
    const localPath = node_path_1.default.isAbsolute(filename) ? new node_url_1.URL(`file://${filename}`) : new node_url_1.URL(filename, `file://${process.cwd()}/`);
    if (!node_fs_1.default.existsSync(localPath)) {
        (0, utils_js_1.error)(`Could not locate ${filename}`);
        process.exit(1);
    }
    else if (node_fs_1.default.statSync(localPath).isDirectory()) {
        (0, utils_js_1.error)(`${localPath} is a directory not a file`);
        process.exit(1);
    }
    return localPath;
}
    ...
    for (const [k, v] of Object.entries(httpHeaders)) {
        if (typeof v === "string") {
            ...
            try {
                ...
                (0, utils_js_1.error)(`Can’t parse key: ${k} into JSON format. Continuing with the next HTTP header that is specified`);
            }
        }
    }
    return finalHeaders;
...
async function load(schema, options) {
    let schemaID = ".";
    if (schema instanceof node_url_1.URL) {
        ...
        const ext = node_path_1.default.extname(schema.pathname).toLowerCase();
        if (schema.protocol.startsWith("http")) {
            ...
            const contents = node_fs_1.default.readFileSync(schema, "utf8");
            if (ext === ".yaml" || ext === ".yml") {
                options.schemas[schemaID] = {
                    hint,
    ...
    else if (schema instanceof node_stream_1.Readable) {
        ...
        (0, utils_js_1.error)(`Invalid schema`);
        process.exit(1);
    }
    ...
    const refPromises = [];
    (0, utils_js_1.walk)(currentSchema, (rawNode, nodePath) => {
            ...
            if (Array.isArray(rawNode[k])) {
                rawNode[k] = rawNode[k].filter((o) => {
                    ...
                    if (!("$ref" in o) || typeof o.$ref !== "string")
                        return true;
                    const ref = (0, utils_js_1.parseRef)(o.$ref);
        ...
        if (!("$ref" in rawNode) || typeof rawNode.$ref !== "string")
            return;
        const node = rawNode;
        const ref = (0, utils_js_1.parseRef)(node.$ref);
        if (ref.filename === ".") {
            return;
        }
        ...
        if ((0, utils_js_1.isRemoteURL)(ref.filename) || (0, utils_js_1.isFilepath)(ref.filename)) {
            const nextURL = new node_url_1.URL(ref.filename.startsWith("//") ? `https://${ref.filename}` : ref.filename);
            refPromises.push(load(nextURL, { ...options, hint }));
            node.$ref = node.$ref.replace(ref.filename, nextURL.href);
            return;
        }
        if (options.rootURL.href === exports.VIRTUAL_JSON_URL) {
            (0, utils_js_1.error)(`Can’t resolve "${ref.filename}" from dynamic JSON. Either load this schema from a filepath/URL, or set the \`cwd\` option: \`openapiTS(schema, { cwd: '/path/to/cwd' })\`.`);
            process.exit(1);
        }
        const nextURL = new node_url_1.URL(ref.filename, schema instanceof node_url_1.URL ? schema : options.rootURL);
        const nextID = relativePath(schema instanceof node_url_1.URL ? schema : options.rootURL, nextURL);
        refPromises.push(load(nextURL, { ...options, hint }));
    ...
    if (schemaID === ".") {
        for (const subschemaID of Object.keys(options.schemas)) {
            (0, utils_js_1.walk)(options.schemas[subschemaID].schema, (rawNode) => {
                if (!("$ref" in rawNode) || typeof rawNode.$ref !== "string")
                    return;
                const node = rawNode;
                const ref = (0, utils_js_1.parseRef)(node.$ref);
                if (ref.filename === ".") {
                    if (subschemaID === "." || ref.path[0] === "external") {
                        node.$ref = (0, utils_js_1.makeTSIndex)(ref.path);
                    }
                    else {
                        node.$ref = (0, utils_js_1.makeTSIndex)(["external", subschemaID, ...ref.path]);
                    }
                }
                else {
                    const refURL = new node_url_1.URL(ref.filename, new node_url_1.URL(subschemaID, options.rootURL));
                    node.$ref = (0, utils_js_1.makeTSIndex)(["external", relativePath(options.rootURL, refURL), ...ref.path]);
                }
            });
        }
    }
    for (const k of Object.keys(options.schemas)) {
        (0, utils_js_1.walk)(options.schemas[k].schema, (rawNode, nodePath) => {
            if (typeof rawNode === "object" && "in" in rawNode) {
                const key = k === "." ? (0, utils_js_1.makeTSIndex)(nodePath) : (0, utils_js_1.makeTSIndex)(["external", k, ...nodePath]);
        ...
        if (JSON.stringify(options.schemas[k].schema).includes('"discriminator"')) {
            (0, utils_js_1.walk)(options.schemas[k].schema, (rawNode, nodePath) => {
                ...
                if (!node.discriminator)
                    return;
                options.discriminators[schemaID === "." ? (0, utils_js_1.makeTSIndex)(nodePath) : (0, utils_js_1.makeTSIndex)(["external", k, ...nodePath])] = node.discriminator;
            });
        }
    }
    return options.schemas;
...
function relativePath(src, dest) {
    ...
    if (isSameOrigin || isSameDisk) {
        return node_path_1.default.posix.relative(node_path_1.default.posix.dirname(src.pathname), dest.pathname);
    }
    return dest.href;
}
function getHint({ path, external, startFrom }) {
